...
#define MQTT_RESTART_TOPIC "Zihan/device/restart"
...
void onMqttConnect(bool sessionPresent) {
  ...
  // Subscribe to the restart topic for this device
  String restartTopic = String(MQTT_RESTART_TOPIC) + String(deviceid);
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  if (topicStr == restartTopic) {
    ...
    restartTimer.once(1, ESP.restart); // Schedule a restart after 1 second
  }
}
